...
void f_ScanInterface(void *args) {
        ...
        const char * ip_address  = f_KeyValue("DeviceIP", "/config/snmp-scan.json");
        ...
        const char * port_str = f_KeyValue("DevicePort", "/config/snmp-scan.json");
        if (!port_str) { ESP_LOGE(TAG, "DevicePort field not found."); vTaskDelete(NULL); return; }
        char *endptr;
        long port = strtol(port_str, &endptr, 10);
        if (endptr == port_str || port <= 0 || port > 65535) { ESP_LOGE(TAG, "Invalid SNMP port: '%s'", port_str); StatusScan = false;vTaskDelete(NULL); return; }
        int timeout_val = f_KeyValueInt("Timeout", "/config/snmp-scan.json");
        ...
        int maxInterface_val = f_KeyValueInt("MaxInterface", "/config/snmp-scan.json");
        ...
        ESP_LOGI(TAG, "Starting SNMP Scan task for %s on port %ld, timeout %d, max-interface %d", ip_address, port, timeout_val, maxInterface_val);
        const char * community = f_KeyValue("Community", "/config/snmp-scan.json");
        ...
        ESP_LOGI(TAG, "Interface listing %s. Discovered interfaces: %d", (result == ESP_OK) ? "completed" : esp_err_to_name(result), discovered_interfaces);
        StatusScan = false;
        vTaskDelete(NULL);
}
...
static void f_SelectInterfaces(void *args) {
            ESP_LOGI(TAG, "Starting SNMP interface selection task");
            vTaskDelay(pdMS_TO_TICKS(500));
            char *json_data = f_lerArquivo("/config/snmp.json"); if (!json_data) { ESP_LOGE(TAG, "Falha ao ler o arquivo JSON"); vTaskDelete(NULL); return; }
            ...
            if (strcmp(snmp_action->valuestring, "substituir") == 0) {
                    if (saveJsonToFile("/config/snmp-interface-select.json", json_novo) == 0) {
                        ESP_LOGI(TAG, "snmp-interface-select.json file created successfully");
                    } else {
                        ESP_LOGE(TAG, "Error saving snmp-interface-select.json file");
                    }
            } else if (strcmp(snmp_action->valuestring, "adicionar") == 0) {
                cJSON *json_existente = read_json_file("/config/snmp-interface-select.json");
                if (!json_existente) {
                    ESP_LOGW(TAG, "Existing file not found. Creating new one.");
                ...
                if (saveJsonToFile("/config/snmp-interface-select.json", json_existente) == 0) {
                    ESP_LOGI(TAG, "snmp-interface-select.json file updated successfully");
                } else {
                    ESP_LOGE(TAG, "Error updating snmp-interface-select.json file");
                }
...
void f_AdicionarCustomOID() {
        ESP_LOGI("CUSTOM_OID", "Starting addition of SNMP Custom OID");

        char *json_str = f_lerArquivo("/config/snmp-custom-oid.json");
        if (!json_str) {
            ESP_LOGE("CUSTOM_OID", "Error reading snmp-custom-oid.json");
            return;
        }
        ...
        safe_free(&json_str);
        if (!json_custom) {
            ESP_LOGE("CUSTOM_OID", "Error parsing custom JSON");
            return;
        }
        ...
        if (!cJSON_IsString(ip_item) || !cJSON_IsString(port_item) || !cJSON_IsString(community_item)) {
            ESP_LOGE("CUSTOM_OID", "IP, Port or Community fields missing or invalid in JSON");
        ...
        cJSON *json_destino = read_json_file("/config/snmp-interface-select.json");
        if (!json_destino) {
            ESP_LOGW("CUSTOM_OID", "snmp-interface-select.json file does not exist. Creating a new one.");
        ...
        if (saveJsonToFile("/config/snmp-interface-select.json", json_destino) == 0) {
            ESP_LOGI("CUSTOM_OID", "snmp-interface-select.json file updated successfully!");
        } else {
            ESP_LOGE("CUSTOM_OID", "Error saving snmp-interface-select.json");
        }
...
void f_startReadSnmp() {
    if (!f_KeyStatus("enable-snmp", "/config/snmp-setup.json")) {return;}
    while (!f_Wifi_Conectado()){vTaskDelay(pdMS_TO_TICKS(100));}
    if (hReadInterface == NULL) {
        xTaskCreate(f_ReadSnmp, "t_SNMP", 6500, NULL, 5, &hReadInterface);
    } else {
        ESP_LOGW(TAG, "SNMP Interface status reading task is already running");
    }
}
...
void f_ReadSnmp(void *args) {
        ...
        ESP_LOGI(TAG, "Ending automatic reading of SNMP interfaces");
        hReadInterface = NULL;
        vTaskDelete(NULL);
}
...
static void debug_dispositivos(IPInfo *dispositivos, int total_ips) {
    ESP_LOGW(TAG, "Starting debug of SNMP devices");
    for (int i = 0; i < total_ips; i++) {
        ESP_LOGI(TAG, "Devices #%d", i);
        ESP_LOGI(TAG, "  IP: %s", dispositivos[i].ip);
        ESP_LOGI(TAG, "  Port: %d", dispositivos[i].port);
        ...
        for (int j = 0; j < dispositivos[i].total_oids; j++) {
            ...
            ESP_LOGI(TAG, "      Type: %d", dispositivos[i].oids[j].tipo); // pode mapear esse enum depois se quiser
        }
    }
    ESP_LOGW(TAG, "End");
}
